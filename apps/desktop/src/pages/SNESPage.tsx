...
function RankBadge({ rank }: { rank: number }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 24, height: 24, borderRadius: '50%', background: '#333', color: '#ccc', fontSize: 12, fontWeight: 700 }}>
      {rank}
    </span>
  );
}
...
function SNESBanner() {
  return (
    <div style={{ background: 'linear-gradient(135deg, #7B5EA7 0%, #5a3f87 100%)', borderRadius: 12, padding: '16px 20px', marginBottom: 20, color: '#fff', fontSize: 13 }}>
      <strong>🎮 SNES · Snes9x Emulator</strong>
      <span style={{ margin: '0 8px', opacity: 0.6 }}>·</span>
      <span>2–4 player online relay</span>
      <span style={{ margin: '0 8px', opacity: 0.6 }}>·</span>
      <span>Automatic .sfc / .smc ROM detection</span>
      <p style={{ marginTop: 6, marginBottom: 0, opacity: 0.85, fontSize: 12 }}>
        Snes9x provides robust netplay. Fallback to bsnes (high accuracy) or RetroArch (snes9x core). Performance preset 'accurate' enables bsnes-level cycle accuracy.
      </p>
    </div>
  );
}
...
function SNESGameCard({
  game, openRooms, onQuickMatch, onHostRoom,
}: { game: MockGame; openRooms: number; onQuickMatch: (game: MockGame) => void; onHostRoom: (game: MockGame) => void }) {
  return (
    <div style={{ background: '#1a1a1a', borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 8, border: '1px solid #2a2a2a' }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        ...
        {openRooms > 0 && (
          <span style={{ background: '#1a1a3a', borderRadius: 4, padding: '2px 6px', fontSize: 11, color: '#7B5EA7' }}>{openRooms} open</span>
        )}
      </div>
      <div style={{ color: '#888', fontSize: 12, lineHeight: 1.4 }}>{game.description}</div>
      {game.maxPlayers > 1 && (
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          <button onClick={() => onQuickMatch(game)} style={{ flex: 1, padding: '8px 0', borderRadius: 6, border: 'none', background: 'linear-gradient(135deg, #7B5EA7, #5a3f87)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ⚡ Quick Match
          </button>
          <button onClick={() => onHostRoom(game)} style={{ flex: 1, padding: '8px 0', borderRadius: 6, border: '1px solid #7B5EA7', background: 'transparent', color: '#7B5EA7', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            🎮 Host Room
          </button>
        </div>
      )}
...
function LeaderboardPanel() {
  ...
  useEffect(() => {
    fetch(`${SERVER_URL}/api/leaderboard?orderBy=sessions&limit=10`)
      .then((r) => r.json())
      .then((data: { leaderboard: SnesRanking[] }) => setRankings(data.leaderboard ?? []))
      .catch(() => {});
  }, []);
  return (
    <div style={{ maxWidth: 500 }}>
      <h3 style={{ margin: '0 0 16px', fontSize: 16, color: '#7B5EA7' }}>🏆 Top SNES Players</h3>
...
function LobbyPanel({ onJoin }: { onJoin: (roomCode: string) => void }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
      {SNES_GAMES.map((game) => (
        <SNESGameCard key={game.id} game={game} openRooms={0} onQuickMatch={handleQuickMatch} onHostRoom={handleHostRoom} />
      ))}
    </div>
  );
}
...
export default function SNESPage() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 800 }}>🎮 Super Nintendo Entertainment System</h1>
      <p style={{ margin: '0 0 20px', color: '#888', fontSize: 14 }}>Classic SNES games — 2–4 player online relay with Snes9x</p>
      <SNESBanner />
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {(['lobby', 'leaderboard'] as ActiveTab[]).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '8px 18px', borderRadius: 6, border: activeTab === tab ? '2px solid #7B5EA7' : '1px solid #333', background: activeTab === tab ? '#7B5EA7' : 'transparent', color: activeTab === tab ? '#fff' : '#aaa', fontWeight: 700, fontSize: 13, cursor: 'pointer', textTransform: 'capitalize' }}>
