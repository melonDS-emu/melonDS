...
function NDSBanner() {
  return (
    <div style={{ background: 'linear-gradient(135deg, #C45200 0%, #8b3a00 100%)', borderRadius: 12, padding: '16px 20px', marginBottom: 20, color: '#fff', fontSize: 13 }}>
      <strong>🎮 NDS · melonDS Emulator</strong>
      <span style={{ margin: '0 8px', opacity: 0.6 }}>·</span>
      <span>Wi-Fi relay + WFC support</span>
      <span style={{ margin: '0 8px', opacity: 0.6 }}>·</span>
      <span>Automatic .nds / .dsi ROM detection</span>
      <p style={{ marginTop: 6, marginBottom: 0, opacity: 0.85, fontSize: 12 }}>
        melonDS emulates DS Wi-Fi and WFC over TCP relay. Supports Wiimmfi DNS for Pokémon and Nintendo online titles. Touch controls via mouse. DeSmuME available as fallback.
      </p>
    </div>
  );
}

function NDSGameCard({ game, openRooms, onQuickMatch, onHostRoom }: { game: MockGame; openRooms: number; onQuickMatch: (g: MockGame) => void; onHostRoom: (g: MockGame) => void }) {
  return (
    <div style={{ background: '#1a1a1a', borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 8, border: '1px solid #2a2a2a' }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        ...
        {openRooms > 0 && <span style={{ background: '#2a1a0a', borderRadius: 4, padding: '2px 6px', fontSize: 11, color: '#E87722' }}>{openRooms} open</span>}
      </div>
      <div style={{ color: '#888', fontSize: 12, lineHeight: 1.4 }}>{game.description}</div>
      {game.maxPlayers > 1 && (
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          <button onClick={() => onQuickMatch(game)} style={{ flex: 1, padding: '8px 0', borderRadius: 6, border: 'none', background: 'linear-gradient(135deg, #C45200, #8b3a00)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>⚡ Quick Match</button>
          <button onClick={() => onHostRoom(game)} style={{ flex: 1, padding: '8px 0', borderRadius: 6, border: '1px solid #E87722', background: 'transparent', color: '#E87722', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>🎮 Host Room</button>
        </div>
      )}
    </div>
  );
...
function LeaderboardPanel() {
  ...
  useEffect(() => {
    fetch(`${SERVER_URL}/api/leaderboard?orderBy=sessions&limit=10`)
      .then((r) => r.json())
      .then((data: { leaderboard: NdsRanking[] }) => setRankings(data.leaderboard ?? []))
      .catch(() => {});
  }, []);
  return (
    <div style={{ maxWidth: 500 }}>
      <h3 style={{ margin: '0 0 16px', fontSize: 16, color: '#E87722' }}>🏆 Top NDS Players</h3>
...
function LobbyPanel({ onJoin }: { onJoin: (roomCode: string) => void }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
      {NDS_GAMES.map((game) => (
        <NDSGameCard key={game.id} game={game} openRooms={0} onQuickMatch={handleQuickMatch} onHostRoom={handleHostRoom} />
      ))}
    </div>
  );
}
...
export default function NDSPage() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 800 }}>🎮 Nintendo DS</h1>
      <p style={{ margin: '0 0 20px', color: '#888', fontSize: 14 }}>DS Wi-Fi games — touch screen, WFC relay, and up to 8-player online via melonDS</p>
      <NDSBanner />
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {(['lobby', 'leaderboard'] as ActiveTab[]).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '8px 18px', borderRadius: 6, border: activeTab === tab ? '2px solid #E87722' : '1px solid #333', background: activeTab === tab ? '#E87722' : 'transparent', color: activeTab === tab ? '#fff' : '#aaa', fontWeight: 700, fontSize: 13, cursor: 'pointer', textTransform: 'capitalize' }}>
