...
const SMS_GAMES: SMSGame[] = [
  { id: 'sms-alex-kidd-in-miracle-world', title: 'Alex Kidd in Miracle World', genre: 'Platformer',  genreEmoji: '👊', maxPlayers: 1, singleOnly: true },
  { id: 'sms-sonic-the-hedgehog',         title: 'Sonic the Hedgehog',         genre: 'Platformer',  genreEmoji: '💨', maxPlayers: 1, singleOnly: true },
  { id: 'sms-wonder-boy-iii',             title: "Wonder Boy III: The Dragon's Trap", genre: 'Action-RPG', genreEmoji: '🐉', maxPlayers: 1, singleOnly: true },
  { id: 'sms-phantasy-star',              title: 'Phantasy Star',              genre: 'RPG',         genreEmoji: '🌌', maxPlayers: 1, singleOnly: true },
  { id: 'sms-golden-axe-warrior',         title: 'Golden Axe Warrior',         genre: 'Action-RPG',  genreEmoji: '⚔️', maxPlayers: 1, singleOnly: true },
  { id: 'sms-r-type',                     title: 'R-Type',                     genre: 'Shoot-em-up', genreEmoji: '🚀', maxPlayers: 1, singleOnly: true },
  { id: 'sms-shinobi',                    title: 'Shinobi',                    genre: 'Action',      genreEmoji: '🥷', maxPlayers: 1, singleOnly: true },
  { id: 'sms-after-burner',               title: 'After Burner',               genre: 'Shoot-em-up', genreEmoji: '✈️', maxPlayers: 1, singleOnly: true },
];
...
function SMSBanner() {
  return (
    <div
      ...
      style={{ backgroundColor: 'rgba(0,51,153,0.1)', border: '1px solid rgba(0,51,153,0.35)' }}
    >
      ...
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--color-oasis-text-muted)' }}>
          SMS sessions use RetroOasis TCP relay netplay with RetroArch and the Genesis Plus
          GX core — the same proven core that handles Genesis and Game Gear. ROM files
          (.sms, .gg, .sg) are auto-detected from your library. No port forwarding required.
          Recommended core:{' '}
          <code
            className="mx-1 px-1 rounded"
            style={{ background: 'rgba(0,51,153,0.2)', color: '#93c5fd' }}
          >
    ...
    >
      ...
      <div className="flex gap-2 mt-auto">
        <button
          ...
          style={{ background: 'linear-gradient(90deg, #003399, #001a66)', color: '#fff' }}
        >
          ⚡ Quick Match
        </button>
        <button
          onClick={() => onHost(game)}
          className="flex-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all hover:scale-105"
          style={{ background: 'rgba(0,51,153,0.12)', border: '1px solid rgba(0,51,153,0.35)', color: '#93c5fd' }}
        >
          🎮 Host Room
        </button>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--color-oasis-text-muted)' }}>
            Relay sessions via RetroArch + Genesis Plus GX · No port forwarding needed
          </p>
        </div>
      </div>

      {/* Tab bar */}
      <div className="flex gap-2 border-b" style={{ borderColor: 'rgba(0,51,153,0.2)' }}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
