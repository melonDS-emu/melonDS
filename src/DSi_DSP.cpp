...
u16 DSi_DSP::GetPSTS() const noexcept
{
...
{
    if (DSP_PCFG & (1<< 9)) DSi.SetIRQ(0, NDS::IRQ_DSi_DSP);
}
void DSi_DSP::IrqRep1()
{
    if (DSP_PCFG & (1<<10)) DSi.SetIRQ(0, NDS::IRQ_DSi_DSP);
}
void DSi_DSP::IrqRep2()
{
    if (DSP_PCFG & (1<<11)) DSi.SetIRQ(0, NDS::IRQ_DSi_DSP);
}
void DSi_DSP::IrqSem()
{
    DSP_PSTS |= 1<<9;
    // apparently these are always fired?
    DSi.SetIRQ(0, NDS::IRQ_DSi_DSP);
}

u16 DSi_DSP::DSPRead16(u32 addr) const noexcept
{
    if (!(addr & 0x40000))
    {
...
{
    DSi.RegisterEventFunc(NDS::Event_DSi_DSP, 0, MemberEventFunc(DSi_DSP, DSPCatchUpU32));
...
DSi_DSP::~DSi_DSP()
{
    ...
    DSi.UnregisterEventFunc(NDS::Event_DSi_DSP, 0);
}
...
void DSi_DSP::Reset()
{
    ...
    DSi.CancelEvent(NDS::Event_DSi_DSP);

    SNDExCnt = 0;
}

bool DSi_DSP::IsRstReleased() const noexcept
{
    return SCFG_RST;
}
...
inline bool DSi_DSP::IsDSPCoreEnabled() const noexcept
{
    return (DSi.SCFG_Clock9 & (1<<1)) && SCFG_RST && (!(DSP_PCFG & (1<<0)));
}

inline bool DSi_DSP::IsDSPIOEnabled() const noexcept
{
...
void DSi_DSP::PDataDMAWrite(u16 wrval)
{
    ...
    DSi.SetIRQ(0, NDS::IRQ_DSi_DSP); // wrfifo empty
}
...
void DSi_DSP::Run(u32 cycles)
{
    ...
    DSi.CancelEvent(NDS::Event_DSi_DSP);
    DSi.ScheduleEvent(NDS::Event_DSi_DSP, false,
