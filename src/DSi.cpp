...
DSi::DSi(NDSSysfileArguments&& ndsSysfiles, DSiSysfileArguments&& dsiSysfiles, const InitArguments& args) noexcept :
    ...
    NDMAs {
        ...
        DSi_NDMA(0, 2, GPU, *this),
        DSi_NDMA(0, 3, GPU, *this),
        DSi_NDMA(1, 0, GPU, *this),
        DSi_NDMA(1, 1, GPU, *this),
        DSi_NDMA(1, 2, GPU, *this),
...
DSi::~DSi() noexcept
{
    ...
    // All objects are cleaned up by their own destructors,
...
void DSi::Stop(Platform::StopReason reason) noexcept
{
    NDS::Stop(reason);
    CamModule.Stop();
}

bool DSi::LoadCart(const u8* romdata, u32 romlen, const u8* savedata, u32 savelen) noexcept
{
...
void DSi::EjectCart() noexcept
{
    NDS::EjectCart();

    SetCartInserted(false);
}
void DSi::CamInputFrame(int cam, const u32* data, int width, int height, bool rgb) noexcept
{
    switch (cam)
    {
...
void DSi::DoSavestateExtra(Savestate* file) noexcept
{
    file->Section("DSIG");
...
void DSi::SetCartInserted(bool inserted) noexcept
{
    if (inserted)
        SCFG_MC &= ~1;
    else
...
void DSi::DecryptModcryptArea(u32 offset, u32 size, u8* iv) noexcept
{
    AES_ctx ctx;
    u8 key[16];
    u8 tmp[16];
...
void DSi::SetupDirectBoot() noexcept
{
    bool dsmode = false;
    ...
    else
    {
        ...
        if (auto* nandimage = SDMMC.GetNAND())
...
bool DSi::LoadNAND() noexcept
{
    ...
    FileHandle* nand = SDMMC.GetNAND()->GetFile();
    ...
    memset(NWRAM_A, 0, DSiNWRAMSize);
    memset(NWRAM_B, 0, DSiNWRAMSize);
    memset(NWRAM_C, 0, DSiNWRAMSize);
    ...
    const DSi_NAND::DSiKey& emmccid = SDMMC.GetNAND()->GetEMMCID();
...
void DSi::RunNDMAs(u32 cpu) noexcept
{
...
void DSi::StallNDMAs() noexcept
{
    // TODO
}


bool DSi::DMAsInMode(u32 cpu, u32 mode) const noexcept
{
...
bool DSi::DMAsRunning(u32 cpu) const noexcept
{
...
bool DSi::NDMAsInMode(u32 cpu, u32 mode) const noexcept
{
    cpu <<= 2;
...
bool DSi::NDMAsRunning(u32 cpu) const noexcept
{
    cpu <<= 2;
...
void DSi::CheckNDMAs(u32 cpu, u32 mode) noexcept
{
    cpu <<= 2;
...
void DSi::StopNDMAs(u32 cpu, u32 mode) noexcept
{
    cpu <<= 2;
...
void DSi::StopDMAs(u32 cpu, u32 mode) noexcept
{
    NDS::StopDMAs(cpu, mode);
    StopNDMAs(cpu, mode);
}

void DSi::CheckDMAs(u32 cpu, u32 mode) noexcept
{
...
void DSi::MapNWRAM_A(u32 num, u8 val) noexcept
{
...
void DSi::MapNWRAM_B(u32 num, u8 val) noexcept
{
...
void DSi::MapNWRAM_C(u32 num, u8 val) noexcept
{
...
void DSi::MapNWRAMRange(u32 cpu, u32 num, u32 val) noexcept
{
...
void DSi::ApplyNewRAMSize(u32 size) noexcept
{
    switch (size)
    {
    case 0:
...
void DSi::Set_SCFG_Clock9(u16 val) noexcept
{
...
void DSi::Set_SCFG_MC(u32 val) noexcept
{
...
u8 DSi::ARM9Read8(u32 addr) noexcept
{
...
u16 DSi::ARM9Read16(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x1;
...
u32 DSi::ARM9Read32(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x3;
...
void DSi::ARM9Write8(u32 addr, u8 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr & 0xFF000000)
    {
...
void DSi::ARM9Write16(u32 addr, u16 val) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x1;
...
void DSi::ARM9Write32(u32 addr, u32 val) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x3;
...
bool DSi::ARM9GetMemRegion(u32 addr, bool write, MemRegion* region) noexcept
{
    assert(ConsoleType == 1);
    switch (addr & 0xFF000000)
    {
...
u8 DSi::ARM7Read8(u32 addr) noexcept
{
...
u16 DSi::ARM7Read16(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x1;
...
u32 DSi::ARM7Read32(u32 addr) noexcept
{
    addr &= ~0x3;
    ...
    if ((addr < 0x00010000) && (!(SCFG_BIOS & (1<<9))))
    {
        ...
        if (ARM7.R[15] >= 0x00010000)
            return 0xFFFFFFFF;
        if (addr < NDS::ARM7BIOSProt && ARM7.R[15] >= NDS::ARM7BIOSProt)
            return 0xFFFFFFFF;
...
void DSi::ARM7Write8(u32 addr, u8 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr & 0xFF800000)
    {
...
void DSi::ARM7Write16(u32 addr, u16 val) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x1;
...
void DSi::ARM7Write32(u32 addr, u32 val) noexcept
{
    assert(ConsoleType == 1);
    addr &= ~0x3;
...
bool DSi::ARM7GetMemRegion(u32 addr, bool write, MemRegion* region) noexcept
{
    assert(ConsoleType == 1);
    switch (addr & 0xFF800000)
    {
    ...
    {
        if (ARM7.R[15] < 0x00010000 && (addr >= NDS::ARM7BIOSProt || ARM7.R[15] < NDS::ARM7BIOSProt))
        {
...
u8 DSi::ARM9IORead8(u32 addr) noexcept
{
    switch (addr)
    {
...
u16 DSi::ARM9IORead16(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
u32 DSi::ARM9IORead32(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
void DSi::ARM9IOWrite8(u32 addr, u8 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
    case 0x04000301:
        ...
        //if (val == 0x80 && ARM9.R[15] == 0xFFFF0268) ARM9.Halt(1);
        return;
...
void DSi::ARM9IOWrite16(u32 addr, u16 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
void DSi::ARM9IOWrite32(u32 addr, u32 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
u8 DSi::ARM7IORead8(u32 addr) noexcept
{
    assert(ConsoleType == 1);
...
u16 DSi::ARM7IORead16(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
u32 DSi::ARM7IORead32(u32 addr) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
void DSi::ARM7IOWrite8(u32 addr, u8 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
void DSi::ARM7IOWrite16(u32 addr, u16 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
...
void DSi::ARM7IOWrite32(u32 addr, u32 val) noexcept
{
    assert(ConsoleType == 1);
    switch (addr)
    {
