...
void SoftRenderer::TextureLookup(u32 texparam, u32 texpal, s16 s, s16 t, u16* color, u8* alpha) const noexcept
{
...
u32 SoftRenderer::AlphaBlend(u32 srccolor, u32 dstcolor, u32 alpha) const noexcept
{
    ...
    if (GPU.GPU3D.GetRenderDispCnt() & (1<<3))
    {
...
u32 SoftRenderer::RenderPixel(Polygon* polygon, u8 vr, u8 vg, u8 vb, s16 s, s16 t)
{
    ...
    if (blendmode == 2)
    {
        if (GPU.GPU3D.GetRenderDispCnt() & (1<<1))
        {
        ...
        else
        {
            ...
            u16 tooncolor = GPU.GPU3D.GetRenderToonTable()[vr >> 1];
    ...
    if ((GPU.GPU3D.GetRenderDispCnt() & (1<<0)) && (((polygon->TexParam >> 26) & 0x7) != 0))
    {
        u8 tr, tg, tb;
    ...
    if ((blendmode == 2) && (GPU.GPU3D.GetRenderDispCnt() & (1<<1)))
    {
        u16 tooncolor = GPU.GPU3D.GetRenderToonTable()[vr >> 1];
...
void SoftRenderer::RenderShadowMaskScanline(RendererPolygon* rp, s32 y)
{
    ...
    if (xstart > xend)
    {
        ...
        if ((polyalpha < 31) || (GPU.GPU3D.GetRenderDispCnt() & (3<<4)))
        {
            l_filledge = true;
            r_filledge = true;
        }
    ...
    if (polyalpha <= GPU.GPU3D.GetRenderAlphaRef()) return;
...
void SoftRenderer::RenderPolygonScanline(RendererPolygon* rp, s32 y)
{
    ...
    if (xstart > xend)
    {
        ...
        if ((polyalpha < 31) || wireframe || (GPU.GPU3D.GetRenderDispCnt() & ((1<<4)|(1<<5))))
        {
            l_filledge = true;
            r_filledge = true;
        }
    ...
    for (; x < xlimit; x++)
    {
        ...
        // alpha test
        if (alpha <= GPU.GPU3D.GetRenderAlphaRef()) continue;

        if (alpha == 31)
        {
            u32 attr = polyattr | edge;

            if (GPU.GPU3D.GetRenderDispCnt() & (1<<4))
            {
    ...
    for (; x < xlimit; x++)
    {
        ...
        if (alpha == 31)
        {
            ...
            if ((GPU.GPU3D.GetRenderDispCnt() & (1<<4)) && (attr & 0xF))
            {
                // anti-aliasing: all edges are rendered
...
u32 SoftRenderer::CalculateFogDensity(u32 pixeladdr) const noexcept
{
    u32 z = DepthBuffer[pixeladdr];
    u32 densityid, densityfrac;

    if (z < GPU.GPU3D.GetRenderFogOffset())
    {
        densityid = 0;
        densityfrac = 0;
    }
    else
    {
        ...
        z -= GPU.GPU3D.GetRenderFogOffset();
        z = (z >> 2) << GPU.GPU3D.GetRenderFogShift();
    ...
    u32 density =
        ((GPU.GPU3D.GetRenderFogDensityTable()[densityid] * (0x20000-densityfrac)) +
         (GPU.GPU3D.GetRenderFogDensityTable()[densityid+1] * densityfrac)) >> 17;
...
void SoftRenderer::ScanlineFinalPass(s32 y)
{
    ...
    if (GPU.GPU3D.GetRenderDispCnt() & (1<<5))
    {
        // edge marking
        ...
        for (int x = 0; x < 256; x++)
        {
            ...
            if (((polyid != (AttrBuffer[pixeladdr-1] >> 24)) && (z < DepthBuffer[pixeladdr-1])) ||
                ...
                ((polyid != (AttrBuffer[pixeladdr+ScanlineWidth] >> 24)) && (z < DepthBuffer[pixeladdr+ScanlineWidth])))
            {
                u16 edgecolor = GPU.GPU3D.GetRenderEdgeTable()[polyid >> 3];
    ...
    if (GPU.GPU3D.GetRenderDispCnt() & (1<<7))
    {
        // fog
        ...
        bool fogcolor = !(GPU.GPU3D.GetRenderDispCnt() & (1<<6));

        u32 fogR = (GPU.GPU3D.GetRenderFogColor() << 1) & 0x3E; if (fogR) fogR++;
        u32 fogG = (GPU.GPU3D.GetRenderFogColor() >> 4) & 0x3E; if (fogG) fogG++;
        u32 fogB = (GPU.GPU3D.GetRenderFogColor() >> 9) & 0x3E; if (fogB) fogB++;
        u32 fogA = (GPU.GPU3D.GetRenderFogColor() >> 16) & 0x1F;
...
{
    u32 clearz = ((GPU.GPU3D.GetRenderClearAttr2() & 0x7FFF) * 0x200) + 0x1FF;
    u32 polyid = GPU.GPU3D.GetRenderClearAttr1() & 0x3F000000; // this sets the opaque polygonID
    ...
    if (GPU.GPU3D.GetRenderDispCnt() & (1<<14))
    {
        u8 xoff = (GPU.GPU3D.GetRenderClearAttr2() >> 16) & 0xFF;
        u8 yoff = (GPU.GPU3D.GetRenderClearAttr2() >> 24) & 0xFF;
    ...
    {
        // TODO: confirm color conversion
        u32 r = (GPU.GPU3D.GetRenderClearAttr1() << 1) & 0x3E; if (r) r++;
        u32 g = (GPU.GPU3D.GetRenderClearAttr1() >> 4) & 0x3E; if (g) g++;
        u32 b = (GPU.GPU3D.GetRenderClearAttr1() >> 9) & 0x3E; if (b) b++;
        u32 a = (GPU.GPU3D.GetRenderClearAttr1() >> 16) & 0x1F;
        u32 color = r | (g << 8) | (b << 16) | (a << 24);

        polyid |= (GPU.GPU3D.GetRenderClearAttr1() & 0x8000);
...
{
    if (RenderThreadRunning.load(std::memory_order_relaxed) && !GPU.GPU3D.GetAbortFrame())
        Platform::Semaphore_Wait(Sema_RenderDone);
}
...
void SoftRenderer::RenderFrame()
{
    ...
    FrameIdentical = !(textureChanged || texPalChanged) && GPU.GPU3D.IsRenderFrameIdentical();
    ...
    {
        ClearBuffers();
        RenderPolygons(false, &GPU.GPU3D.GetRenderPolygonRAM()[0], GPU.GPU3D.GetRenderNumPolygons());
    }
}
...
void SoftRenderer::RenderThreadFunc()
{
    for (;;)
    {
        ...
        {
            ClearBuffers();
            RenderPolygons(true, GPU.GPU3D.GetRenderPolygonRAM().data(), GPU.GPU3D.GetRenderNumPolygons());
        }
