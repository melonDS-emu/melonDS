...
bool LoadROM(const char* path, const char* sram, bool direct)
{
    ...
    bool IsArchive = Platform::CheckArchiveExtensions(ext);

    FILE* f;
    u8 *extractBuf; size_t extractSize;

    if(IsArchive)
    {
        extractBuf = Platform::OpenArchive(path, &extractSize);
        if(!extractBuf)
            return false;
    }
    else
    {
        f = Platform::OpenFile(path, "rb");
        if (!f)
        {
            return false;
        }
