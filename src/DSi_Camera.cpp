...
DSi_CamModule::~DSi_CamModule()
{
    ...
    DSi.UnregisterEventFunc(NDS::Event_DSi_CamIRQ, 0);
    DSi.UnregisterEventFunc(NDS::Event_DSi_CamTransfer, 0);
}
...
void DSi_CamModule::Reset()
{
    ...
    DSi.ScheduleEvent(NDS::Event_DSi_CamIRQ, false, kIRQInterval, 0, 0);
}
...
void DSi_CamModule::IRQ(u32 param)
{
    ...
    if (activecam)
    {
        ...
        if (Cnt & (1<<11))
            DSi.SetIRQ(0, NDS::IRQ_DSi_Camera);
        ...
        if (Cnt & (1<<15))
        {
            ...
            BufferNumLines = 0;
            CurCamera = activecam;
            DSi.ScheduleEvent(NDS::Event_DSi_CamTransfer, false, kTransferStart, 0, 0);
        }
    }

    DSi.ScheduleEvent(NDS::Event_DSi_CamIRQ, true, kIRQInterval, 0, 0);
}
    ...
    if (Cnt & (1<<14))
    {
        ...
        if (line < ystart || line > yend)
        {
            if (!CurCamera->TransferDone())
                DSi.ScheduleEvent(NDS::Event_DSi_CamTransfer, false, delay, 0, line+1);
