...
{
    if (!(Cnt & (1<<13))) Platform::LogMsg("CAMERA: !! REQUESTING YUV FRAME\n");
...
void DSi_Camera::Transfer(u32 pos)
{
    ...
    // TODO: present data
    //Platform::LogMsg("CAM TRANSFER POS=%d/%d\n", pos, 0x6000*2);
...
u8 DSi_Camera::I2C_Read(bool last)
{
    ...
    if (DataPos < 2)
    {
        Platform::LogMsg("DSi_Camera: WHAT??\n");
        ret = 0;
    }
    else
    {
...
void DSi_Camera::I2C_Write(u8 val, bool last)
{
    if (DataPos < 2)
    {
        ...
        if (RegAddr & 0x1) Platform::LogMsg("DSi_Camera: !! UNALIGNED REG ADDRESS %04X\n", RegAddr);
    }
    else
    {
        if (DataPos & 0x1)
...
u16 DSi_Camera::I2C_ReadReg(u16 addr)
{
    ...
    if(Num==1)Platform::LogMsg("DSi_Camera%d: unknown read %04X\n", Num, addr);
    return 0;
}
...
void DSi_Camera::I2C_WriteReg(u16 addr, u16 val)
{
    ...
    case 0x0018:
        ...
        val |= ((val & 0x0001) << 14);
        StandbyCnt = val;
        Platform::LogMsg("CAM%d STBCNT=%04X (%04X)\n", Num, StandbyCnt, val);
        return;
    case 0x001A:
        MiscCnt = val & 0x0B7B;
        Platform::LogMsg("CAM%d MISCCNT=%04X (%04X)\n", Num, MiscCnt, val);
        return;
    }

    if(Num==1)Platform::LogMsg("DSi_Camera%d: unknown write %04X %04X\n", Num, addr, val);
}
...
u8 DSi_Camera::Read8(u32 addr)
{
    ...
    Platform::LogMsg("unknown DSi cam read8 %08X\n", addr);
    return 0;
}
...
u16 DSi_Camera::Read16(u32 addr)
{
    ...
    Platform::LogMsg("unknown DSi cam read16 %08X\n", addr);
    return 0;
}
...
u32 DSi_Camera::Read32(u32 addr)
{
    ...
    Platform::LogMsg("unknown DSi cam read32 %08X\n", addr);
    return 0;
}
...
void DSi_Camera::Write8(u32 addr, u8 val)
{
    ...
    Platform::LogMsg("unknown DSi cam write8 %08X %02X\n", addr, val);
}
...
void DSi_Camera::Write16(u32 addr, u16 val)
{
    ...
    Platform::LogMsg("unknown DSi cam write16 %08X %04X\n", addr, val);
}
...
void DSi_Camera::Write32(u32 addr, u32 val)
{
    ...
    Platform::LogMsg("unknown DSi cam write32 %08X %08X\n", addr, val);
}
