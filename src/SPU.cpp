...
constexpr std::array<s16, 0x100> InterpCos = [] {
    std::array<s16, 0x100> result {};
    ...
    for (int i = 0; i < 0x100; i++)
    {
        ...
        result[i] = (s16)(ratio * 0x2000);
    }

    return result;
}();

constexpr array2<s16, 0x100, 4> InterpCubic = [] {
    array2<s16, 0x100, 4> result {};
    for (int i = 0; i < 0x100; i++)
    {
        s32 i1 = i << 6;
        s32 i2 = (i * i) >> 2;
        ...
        result[i][0] = -i3 + 2*i2 - i1;
        result[i][1] = i3 - 2*i2 + 0x4000;
        result[i][2] = -i3 + i2 + i1;
        result[i][3] = i3 - i2;
    }
...
u8 SPU::Read8(u32 addr) const
{
    if (addr < 0x04000500)
    {
        const SPUChannel& chan = Channels[(addr >> 4) & 0xF];
