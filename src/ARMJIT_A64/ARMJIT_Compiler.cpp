...
Compiler::Compiler()
{
    ...
    while (JitRXStart != NULL)
    {
        ...
        if (i++ > 8)
        {
            Platform::LogMsg("couldn't find unmapped place for jit memory\n");
            JitRXStart = NULL;
        }
    }
...
JitBlockEntry Compiler::CompileBlock(ARM* cpu, bool thumb, FetchedInstr instrs[], int instrsCount, bool hasMemInstr)
{
    ...
    {
        Platform::LogMsg("JIT near memory full, resetting...\n");
        ResetBlockCache();
    }
    if ((JitMemMainSize +  JitMemSecondarySize) - OtherCodeRegion < 1024 * 8)
    {
        Platform::LogMsg("JIT far memory full, resetting...\n");
        ResetBlockCache();
    }
