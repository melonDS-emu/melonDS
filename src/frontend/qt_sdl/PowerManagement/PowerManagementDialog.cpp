...
PowerManagementDialog::PowerManagementDialog(QWidget* parent, EmuThread* emuThread) : QDialog(parent), emuThread(emuThread), ui(new Ui::PowerManagementDialog)
{
    ...
    if (emuThread->NDS->ConsoleType == 1)
    {
        ...
        oldDSiBatteryLevel = dsi.I2C.BPTWL.GetBatteryLevel();
        oldDSiBatteryCharging = dsi.I2C.BPTWL.GetBatteryCharging();
    }
    else
    {
    ...
    if (emuThread->NDS->ConsoleType == 1)
    {
        const auto& dsi = static_cast<const DSi&>(*emuThread->NDS);
        ui->cbDSiBatteryCharging->setChecked(dsi.I2C.BPTWL.GetBatteryCharging());
        int dsiBatterySliderPos = 4;
        switch (dsi.I2C.BPTWL.GetBatteryLevel())
        {
...
void PowerManagementDialog::done(int r)
{
    if (r == QDialog::Accepted)
    {
        ...
        {
            const auto& dsi = static_cast<const DSi&>(*emuThread->NDS);
            Config::DSiBatteryLevel = dsi.I2C.BPTWL.GetBatteryLevel();
            Config::DSiBatteryCharging = dsi.I2C.BPTWL.GetBatteryCharging();
        }
        else
        {
    ...
    else
    {
        ...
        {
            auto& dsi = static_cast<DSi&>(*emuThread->NDS);
            dsi.I2C.BPTWL.SetBatteryLevel(oldDSiBatteryLevel);
            dsi.I2C.BPTWL.SetBatteryCharging(oldDSiBatteryCharging);
        }
        else
        {
...
void PowerManagementDialog::on_cbDSiBatteryCharging_toggled()
{
    ...
    {
        auto& dsi = static_cast<DSi&>(*emuThread->NDS);
        dsi.I2C.BPTWL.SetBatteryCharging(ui->cbDSiBatteryCharging->isChecked());
    }
}
...
void PowerManagementDialog::on_sliderDSiBatteryLevel_valueChanged(int value)
{
    ...
    if (emuThread->NDS->ConsoleType == 1)
    {
        ...
        dsi.I2C.BPTWL.SetBatteryLevel(newBatteryLevel);
    }
