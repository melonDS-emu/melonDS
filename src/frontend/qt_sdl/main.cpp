...
void EmuThread::run()
{
    ...
    while (EmuRunning != emuStatus_Exit)
    {
        ...
        if (NDS->ConsoleType == 1)
        {
            ...
            {
                dsi.I2C.BPTWL.SetPowerButtonHeld(currentTime);
            }
            else if (Input::HotkeyReleased(HK_PowerButton))
            {
                dsi.I2C.BPTWL.SetPowerButtonReleased(currentTime);
            }

            // Handle volume buttons
            if (Input::HotkeyDown(HK_VolumeUp))
            {
                dsi.I2C.BPTWL.SetVolumeSwitchHeld(DSi_BPTWL::volumeKey_Up);
            }
            else if (Input::HotkeyReleased(HK_VolumeUp))
            {
                dsi.I2C.BPTWL.SetVolumeSwitchReleased(DSi_BPTWL::volumeKey_Up);
            }

            if (Input::HotkeyDown(HK_VolumeDown))
            {
                dsi.I2C.BPTWL.SetVolumeSwitchHeld(DSi_BPTWL::volumeKey_Down);
            }
            else if (Input::HotkeyReleased(HK_VolumeDown))
            {
                dsi.I2C.BPTWL.SetVolumeSwitchReleased(DSi_BPTWL::volumeKey_Down);
            }

            dsi.I2C.BPTWL.ProcessVolumeSwitchInput(currentTime);
        }
        ...
        if (EmuRunning == emuStatus_Running || EmuRunning == emuStatus_FrameStep)
        {
            ...
            {
                DSi& dsi = static_cast<DSi&>(*NDS);
                u8 volumeLevel = dsi.I2C.BPTWL.GetVolumeLevel();
...
#ifdef OGLRENDERER_ENABLED
    if (auto* gl = dynamic_cast<GLRenderer*>(&NDS->GPU.GetRenderer3D()))
    {
        // hardware-accelerated render
        gl->GetCompositor().BindOutputTexture(frontbuf);
    }
    else
#endif
    {
