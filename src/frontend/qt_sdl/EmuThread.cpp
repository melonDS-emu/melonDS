            ...
            if (isFocused) {
                ...
                if (isInGame) {
                    ...
                    */
                    ...
                    // ホットキーマッピングテーブル（ジャンプテーブル最適化用）
                    static constexpr struct {
                        int hotkey;
                        uint8_t weapon;
                        uint8_t priority;  // 優先度（低い値が高優先）
                    } HOTKEY_MAP[] = {
                        ...
                        { HK_MetroidWeaponSpecial, 0xFF, 8 }  // 特殊処理用
                    };

                    // 武器が切り替えられたかを追跡
                    bool weaponSwitched = false;

                    // 全ホットキー状態を一度に取得（SIMD風の並列化）
