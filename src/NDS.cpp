...
void NDS::SetARM9RegionTimings(u32 addrstart, u32 addrend, u32 region, int buswidth, int nonseq, int seq) noexcept
{
    addrstart >>= 2;
    addrend   >>= 2;
...
void NDS::SetARM7RegionTimings(u32 addrstart, u32 addrend, u32 region, int buswidth, int nonseq, int seq) noexcept
{
    addrstart >>= 3;
    addrend   >>= 3;
...
void NDS::SetupDirectBoot(const std::string& romname) noexcept
{
    const NDSHeader& header = NDSCartSlot.GetCart()->GetHeader();
    SetupDirectBoot();
...
void NDS::Stop(Platform::StopReason reason) noexcept
{
...
bool NDS::DoSavestate(Savestate* file) noexcept
{
    file->Section("NDSG");
    ...
    {
        // HACK
        dynamic_cast<DSi*>(this)->DoSavestate(file);
    }
...
bool NDS::LoadCart(const u8* romdata, u32 romlen, const u8* savedata, u32 savelen) noexcept
{
...
void NDS::LoadSave(const u8* savedata, u32 savelen) noexcept
{
    if (savedata && savelen)
        NDSCartSlot.LoadSave(savedata, savelen);
}

void NDS::EjectCart() noexcept
{
    NDSCartSlot.EjectCart();
}

bool NDS::CartInserted() const noexcept
{
    return NDSCartSlot.GetCart() != nullptr;
}

bool NDS::LoadGBACart(const u8* romdata, u32 romlen, const u8* savedata, u32 savelen) noexcept
{
...
void NDS::LoadGBAAddon(int type) noexcept
{
    GBACartSlot.LoadAddon(type);
}

void NDS::EjectGBACart() noexcept
{
    GBACartSlot.EjectCart();
}

void NDS::LoadBIOS() noexcept
{
    Reset();
}
...
void NDS::RunSystem(u64 timestamp) noexcept
{
...
u64 NDS::NextTargetSleep() const noexcept
{
    u64 minEvent = UINT64_MAX;
...
void NDS::RunSystemSleep(u64 timestamp) noexcept
{
...
void NDS::Reschedule(u64 target) noexcept
{
    if (CurCPU == 0)
    {
...
void NDS::RegisterEventFunc(u32 id, u32 funcid, const EventFunc& func) noexcept
{
...
void NDS::UnregisterEventFunc(u32 id, u32 funcid) noexcept
{
    SchedEvent& evt = SchedList[id];
...
void NDS::ScheduleEvent(u32 id, bool periodic, s32 delay, u32 funcid, u32 param) noexcept
{
...
void NDS::CancelEvent(u32 id) noexcept
{
    SchedListMask &= ~(1<<id);
}
...
void NDS::CheckKeyIRQ(u32 cpu, u32 oldkey, u32 newkey) noexcept
{
    u16 cnt = KeyCnt[cpu];
...
void NDS::SetKeyMask(u32 mask) noexcept
{
...
bool NDS::IsLidClosed() const noexcept
{
    if (KeyInput & (1<<23)) return true;
    return false;
}

void NDS::SetLidClosed(bool closed) noexcept
{
    if (closed)
    {
        KeyInput |= (1<<23);
...
void NDS::MapSharedWRAM(u8 val) noexcept
{
    if (val == WRAMCnt)
        return;
...
void NDS::UpdateWifiTimings() noexcept
{
...
void NDS::SetWifiWaitCnt(u16 val) noexcept
{
...
void NDS::SetGBASlotTimings() noexcept
{
    const int ntimings[4] = {10, 8, 6, 18};
...
void NDS::UpdateIRQ(u32 cpu) noexcept
{
...
void NDS::SetIRQ(u32 cpu, u32 irq) noexcept
{
    IF[cpu] |= (1 << irq);
    UpdateIRQ(cpu);
...
void NDS::ClearIRQ(u32 cpu, u32 irq) noexcept
{
    IF[cpu] &= ~(1 << irq);
    UpdateIRQ(cpu);
}

void NDS::SetIRQ2(u32 irq) noexcept
{
    IF2 |= (1 << irq);
    UpdateIRQ(1);
}

void NDS::ClearIRQ2(u32 irq) noexcept
{
    IF2 &= ~(1 << irq);
    UpdateIRQ(1);
}

bool NDS::HaltInterrupted(u32 cpu) const noexcept
{
    if (cpu == 0)
    {
        if (!(IME[0] & 0x1))
...
void NDS::StopCPU(u32 cpu, u32 mask) noexcept
{
    if (cpu)
    {
        CPUStop |= (mask << 16);
...
void NDS::ResumeCPU(u32 cpu, u32 mask) noexcept
{
    if (cpu) mask <<= 16;
    CPUStop &= ~mask;
}

void NDS::GXFIFOStall() noexcept
{
...
void NDS::GXFIFOUnstall() noexcept
{
    CPUStop &= ~CPUStop_GXStall;
}

void NDS::EnterSleepMode() noexcept
{
...
u32 NDS::GetPC(u32 cpu) const noexcept
{
    return cpu ? ARM7.R[15] : ARM9.R[15];
}

u64 NDS::GetSysClockCycles(int num) noexcept
{
    u64 ret;
...
void NDS::NocashPrint(u32 ncpu, u32 addr) noexcept
{
    // addr: debug string
...
void NDS::MonitorARM9Jump(u32 addr) noexcept
{
...
void NDS::HandleTimerOverflow(u32 tid) noexcept
{
...
void NDS::RunTimer(u32 tid, s32 cycles) noexcept
{
    Timer* timer = &Timers[tid];
...
void NDS::RunTimers(u32 cpu) noexcept
{
...
u16 NDS::TimerGetCounter(u32 timer) noexcept
{
    RunTimers(timer>>2);
...
void NDS::TimerStart(u32 id, u16 cnt) noexcept
{
...
bool NDS::DMAsInMode(u32 cpu, u32 mode) const noexcept
{
    cpu <<= 2;
...
bool NDS::DMAsRunning(u32 cpu) const noexcept
{
    cpu <<= 2;
...
void NDS::CheckDMAs(u32 cpu, u32 mode) noexcept
{
    cpu <<= 2;
...
void NDS::StopDMAs(u32 cpu, u32 mode) noexcept
{
    cpu <<= 2;
...
void NDS::DivDone(u32 param) noexcept
{
    DivCnt &= ~0xC000;
...
void NDS::SqrtDone(u32 param) noexcept
{
    u64 val;
    u32 res = 0;
    u64 rem = 0;
...
void NDS::debug(u32 param) noexcept
{
...
u8 NDS::ARM9Read8(u32 addr) noexcept
{
...
u16 NDS::ARM9Read16(u32 addr) noexcept
{
    addr &= ~0x1;
...
u32 NDS::ARM9Read32(u32 addr) noexcept
{
    addr &= ~0x3;
...
void NDS::ARM9Write8(u32 addr, u8 val) noexcept
{
    switch (addr & 0xFF000000)
    {
    case 0x02000000:
...
void NDS::ARM9Write16(u32 addr, u16 val) noexcept
{
    addr &= ~0x1;
...
void NDS::ARM9Write32(u32 addr, u32 val) noexcept
{
    addr &= ~0x3;
...
bool NDS::ARM9GetMemRegion(u32 addr, bool write, MemRegion* region) noexcept
{
    switch (addr & 0xFF000000)
    {
    case 0x02000000:
...
u8 NDS::ARM7Read8(u32 addr) noexcept
{
    if (addr < 0x00004000)
    {
...
u16 NDS::ARM7Read16(u32 addr) noexcept
{
    addr &= ~0x1;
...
u32 NDS::ARM7Read32(u32 addr) noexcept
{
    addr &= ~0x3;
...
void NDS::ARM7Write8(u32 addr, u8 val) noexcept
{
    switch (addr & 0xFF800000)
    {
    case 0x02000000:
...
void NDS::ARM7Write16(u32 addr, u16 val) noexcept
{
    addr &= ~0x1;
...
void NDS::ARM7Write32(u32 addr, u32 val) noexcept
{
    addr &= ~0x3;
...
bool NDS::ARM7GetMemRegion(u32 addr, bool write, MemRegion* region) noexcept
{
    switch (addr & 0xFF800000)
    {
    case 0x02000000:
...
u8 NDS::ARM9IORead8(u32 addr) noexcept
{
    switch (addr)
    {
...
u16 NDS::ARM9IORead16(u32 addr) noexcept
{
    switch (addr)
    {
...
u32 NDS::ARM9IORead32(u32 addr) noexcept
{
    switch (addr)
    {
...
void NDS::ARM9IOWrite8(u32 addr, u8 val) noexcept
{
    switch (addr)
    {
    case 0x0400006C:
...
void NDS::ARM9IOWrite16(u32 addr, u16 val) noexcept
{
    switch (addr)
    {
...
void NDS::ARM9IOWrite32(u32 addr, u32 val) noexcept
{
    switch (addr)
    {
    case 0x04000004:
...
u8 NDS::ARM7IORead8(u32 addr) noexcept
{
    switch (addr)
    {
...
u16 NDS::ARM7IORead16(u32 addr) noexcept
{
    switch (addr)
    {
...
u32 NDS::ARM7IORead32(u32 addr) noexcept
{
    switch (addr)
    {
...
void NDS::ARM7IOWrite8(u32 addr, u8 val) noexcept
{
    switch (addr)
    {
    case 0x04000132:
...
void NDS::ARM7IOWrite16(u32 addr, u16 val) noexcept
{
    switch (addr)
    {
...
void NDS::ARM7IOWrite32(u32 addr, u32 val) noexcept
{
    switch (addr)
    {
    case 0x04000004:
