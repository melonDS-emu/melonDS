        ...
        else
        {
            ...
            while (Running && GPU.TotalScanlines==0)
            {
                ...
                //while (ARM7Timestamp < ARM7Target)
                {
                    ...
                    target = ARM7Timestamp << ARM9ClockShift;//std::max(std::max(ARM9Timestamp, DMA9Timestamp) >> ARM9ClockShift, A9ContentionTS);
                    if (target <= std::max(std::max(ARM9Timestamp, DMA9Timestamp), A9ContentionTS << ARM9ClockShift)) target = std::max(std::max(ARM9Timestamp, DMA9Timestamp), A9ContentionTS << ARM9ClockShift) + 1;

                    while (std::max(std::max(ARM9Timestamp, DMA9Timestamp), A9ContentionTS << ARM9ClockShift) < target)
                    {
                        ARM9Target = target;
                        CurCPU = 0;
                        RunTimers(0);
                ...
                RunSystem(target>>ARM9ClockShift);
