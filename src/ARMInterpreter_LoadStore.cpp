...
#define A_LDRB \
    ...
    cpu->AddCycles_CDI(); \
    cpu->R[(cpu->CurInstr>>12) & 0xF] = val; \
    if (((cpu->CurInstr>>12) & 0xF) == 15) Platform::LogMsg("!! LDRB PC %08X\n", cpu->R[15]); \
...
#define A_LDRD \
    ...
    if (r&1) { r--; Platform::LogMsg("!! MISALIGNED LDRD %d\n", r+1); } \
...
#define A_LDRD_POST \
    ...
    cpu->R[(cpu->CurInstr>>16) & 0xF] += offset; \
    u32 r = (cpu->CurInstr>>12) & 0xF; \
    if (r&1) { r--; Platform::LogMsg("!! MISALIGNED LDRD_POST %d\n", r+1); } \
...
#define A_STRD \
    ...
    if (cpu->CurInstr & (1<<21)) cpu->R[(cpu->CurInstr>>16) & 0xF] = offset; \
    u32 r = (cpu->CurInstr>>12) & 0xF; \
    if (r&1) { r--; Platform::LogMsg("!! MISALIGNED STRD %d\n", r+1); } \
...
#define A_STRD_POST \
    ...
    cpu->R[(cpu->CurInstr>>16) & 0xF] += offset; \
    u32 r = (cpu->CurInstr>>12) & 0xF; \
    if (r&1) { r--; Platform::LogMsg("!! MISALIGNED STRD_POST %d\n", r+1); } \
...
#define A_LDRH \
    ...
    cpu->AddCycles_CDI(); \
    if (((cpu->CurInstr>>12) & 0xF) == 15) Platform::LogMsg("!! LDRH PC %08X\n", cpu->R[15]); \
...
#define A_LDRSB \
    ...
    cpu->AddCycles_CDI(); \
    if (((cpu->CurInstr>>12) & 0xF) == 15) Platform::LogMsg("!! LDRSB PC %08X\n", cpu->R[15]); \
...
#define A_LDRSH \
    ...
    cpu->AddCycles_CDI(); \
    if (((cpu->CurInstr>>12) & 0xF) == 15) Platform::LogMsg("!! LDRSH PC %08X\n", cpu->R[15]); \
