...
#ifndef GPU3D_COMPUTEVULKAN_SHADERS
#define GPU3D_COMPUTEVULKAN_SHADERS

#include <string>

// Vulkan port of GPU3D_Compute_shaders.h. The rasterisation algorithm is
// identical; the differences are purely in resource declarations:
//  - every resource lives in descriptor set 0 with a unique binding
//    (set 1, binding 0 is the per-variant polygon texture)
//  - the rgba16ui image buffer of span indices became a plain SSBO
//  - the per-variant loose uniforms became push constants
//
// descriptor set 0 layout:
//   0: MetaUniform (UBO)
//   1: Polygons          2: XSpanSetups   3: YSpanSetups
//   4: BinResult         5: WorkDescs
//   6: ColorTiles        7: DepthTiles    8: AttrTiles
//   9: Result           10: SetupIndices
//  11: FinalFB (rgba8 storage image)
//  12: ClearBitmapColor 13: ClearBitmapDepth (usampler2D)
//  14: Capture128       15: Capture256 (sampler2DArray)

namespace melonDS
{

namespace ComputeRendererShadersVulkan
{
...
layout (std430, set = 0, binding = 4) buffer BinResultBuffer
{
...
layout (std430, set = 0, binding = 5) buffer WorkDescBuffer
{
...
layout (std430, set = 0, binding = 6) buffer ColorTileBuffer
{
    uint ColorTiles[];
};
layout (std430, set = 0, binding = 7) buffer DepthTileBuffer
{
    uint DepthTiles[];
};
layout (std430, set = 0, binding = 8) buffer AttrTileBuffer
{
    uint AttrTiles[];
};
...
layout (std430, set = 0, binding = 9) buffer ResultBuffer
{
    uint ResultValue[];
};
...
void main()
{
    uvec2 packedSetup = SetupIndicesData[gl_GlobalInvocationID.x];
    uvec4 setup = uvec4(packedSetup.x & 0xFFFFU, packedSetup.x >> 16,
                        packedSetup.y & 0xFFFFU, packedSetup.y >> 16);
...
layout (set = 0, binding = 14) uniform sampler2DArray Capture128Texture;
layout (set = 0, binding = 15) uniform sampler2DArray Capture256Texture;
...
layout (set = 0, binding = 12) uniform usampler2D ClearBitmapColor;
layout (set = 0, binding = 13) uniform usampler2D ClearBitmapDepth;
