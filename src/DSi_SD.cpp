...
DSi_SDHost::DSi_SDHost(melonDS::DSi& dsi,  std::optional<FATStorage>&& sdcard, DSi_NAND::NANDImage&& nand) noexcept : DSi(dsi), Num(0)
{
    DSi.RegisterEventFunc(NDS::Event_DSi_SDMMCTransfer,
...
{
    DSi.RegisterEventFunc(NDS::Event_DSi_SDIOTransfer ,
                           Transfer_TX, MemberEventFunc(DSi_SDHost, FinishTX));
    DSi.RegisterEventFunc(NDS::Event_DSi_SDIOTransfer,
                           Transfer_RX, MemberEventFunc(DSi_SDHost, FinishRX));
...
DSi_SDHost::~DSi_SDHost()
{
    ...
    // The unique_ptr destructors will delete the objects for us
}
...
void DSi_SDHost::SetCardIRQ()
{
    ...
    const std::unique_ptr<DSi_SDDevice>& dev = Ports[PortSelect & 0x1];
...
{
    const unique_ptr<DSi_SDDevice>& dev = Ports[PortSelect & 0x1];
...
{
    unique_ptr<DSi_SDDevice>& dev = Ports[PortSelect & 0x1];
