...
void DMA::Reset()
{
    ...
    InProgress = false;
    MRAMBurstCount = 0;
    memcpy(MRAMBurstTable.data(), DMATiming::MRAMDummy.data(), sizeof(MRAMBurstTable));
}
    ...
    if (src_rgn == Mem9_MainRAM)
    {
        ...
        if (SrcAddrInc > 0)
        {
            if (burststart || MRAMBurstTable[MRAMBurstCount] == 0)
            {
                ...
                {
                    if (dst_s == 4)
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead16Bursts[1].data(), sizeof(MRAMBurstTable));
                    else
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead16Bursts[2].data(), sizeof(MRAMBurstTable));
                }
                else
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead16Bursts[0].data(), sizeof(MRAMBurstTable));
            }
    ...
    else if (dst_rgn == Mem9_MainRAM)
    {
        if (DstAddrInc > 0)
        {
            if (burststart || MRAMBurstTable[MRAMBurstCount] == 0)
            {
                ...
                {
                    if (src_s == 4)
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite16Bursts[1].data(), sizeof(MRAMBurstTable));
                    else
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite16Bursts[2].data(), sizeof(MRAMBurstTable));
                }
                else
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite16Bursts[0].data(), sizeof(MRAMBurstTable));
            }
    ...
    if (src_rgn == Mem9_MainRAM)
    {
        ...
        if (SrcAddrInc > 0)
        {
            if (burststart || MRAMBurstTable[MRAMBurstCount] == 0)
            {
                ...
                {
                    if (dst_s == 8)
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead32Bursts[2].data(), sizeof(MRAMBurstTable));
                    else
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead32Bursts[3].data(), sizeof(MRAMBurstTable));
                }
                else if (dst_n == 2)
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead32Bursts[0].data(), sizeof(MRAMBurstTable));
                else
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMRead32Bursts[1].data(), sizeof(MRAMBurstTable));
            }
    ...
    else if (dst_rgn == Mem9_MainRAM)
    {
        if (DstAddrInc > 0)
        {
            if (burststart || MRAMBurstTable[MRAMBurstCount] == 0)
            {
                ...
                {
                    if (src_s == 8)
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite32Bursts[2].data(), sizeof(MRAMBurstTable));
                    else
                        memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite32Bursts[3].data(), sizeof(MRAMBurstTable));
                }
                else if (src_n == 2)
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite32Bursts[0].data(), sizeof(MRAMBurstTable));
                else
                    memcpy(MRAMBurstTable.data(), DMATiming::MRAMWrite32Bursts[1].data(), sizeof(MRAMBurstTable));
            }
