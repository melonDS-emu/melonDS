...
DSi_NWifi::DSi_NWifi(melonDS::DSi& dsi, DSi_SDHost* host) :
    ...
    DSi(dsi)
{
    DSi.RegisterEventFunc(NDS::Event_DSi_NWifi, 0, MemberEventFunc(DSi_NWifi, MSTimer));
...
DSi_NWifi::~DSi_NWifi()
{
    DSi.CancelEvent(NDS::Event_DSi_NWifi);

    DSi.UnregisterEventFunc(NDS::Event_DSi_NWifi, 0);
}
    ...
    case 0x0004: // setup complete
        {
            ...
            *(u32*)&regdomain_evt[0] = 0x80000000 | (*(u16*)&EEPROM[0x008] & 0x0FFF);
            ...
            BootPhase = 2;
            DSi.ScheduleEvent(NDS::Event_DSi_NWifi, false, 33611, 0, 0);
        }
        break;
...
void DSi_NWifi::MSTimer(u32 param)
{
    ...
    DSi.ScheduleEvent(NDS::Event_DSi_NWifi, true, 33611, 0, 0);
}
