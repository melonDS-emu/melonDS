...
#undef SETVERTEX
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(CompVertices), CompVertices.data(), GL_STATIC_DRAW);
    ...
    glGenFramebuffers(CompScreenOutputFB.size(), CompScreenOutputFB.data());
    ...
    glGenTextures(CompScreenOutputTex.size(), CompScreenOutputTex.data());
...
GLCompositor::~GLCompositor()
{
    ...
    glDeleteFramebuffers(CompScreenOutputFB.size(), CompScreenOutputFB.data());
    glDeleteTextures(1, &CompScreenInputTex);
    glDeleteTextures(CompScreenOutputTex.size(), CompScreenOutputTex.data());
