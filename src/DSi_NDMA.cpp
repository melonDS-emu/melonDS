...
void DSi_NDMA::WriteCnt(u32 val)
{
    ...
    if ((!(oldcnt & 0x80000000)) && (val & 0x80000000)) // checkme
    {
        ...
        case 3: DstAddrInc = 1; Platform::LogMsg("BAD NDMA DST INC MODE 3\n"); break;
        }
        ...
        if (StartMode != 0x10 && StartMode != 0x30 &&
            ...
            Platform::LogMsg("UNIMPLEMENTED ARM%d NDMA%d START MODE %02X, %08X->%08X LEN=%d BLK=%d CNT=%08X\n",
...
void DSi_NDMA::Start()
{
    ...
    //    Platform::LogMsg("TODO! NDMA SUBBLOCK TIMER: %08X\n", SubblockTimer);
